...
from common import List, make_dirs, Tuple
...
class ActorCriticIndependent(AgentInterface, ActorCriticInterface):
    ...
    def __init__(
        ...
        self.cnets = [CriticNetwork(self.n_features) for _ in range(n_players)]
        self.anets = [
            ActorNetwork(self.n_features, self.n_actions) for _ in range(n_players)
        ]
        self.zeta = zeta
    ...
    @cached_property
    def aopts(self) -> List[Optimizer]:
        """Actor optimizer"""
        return [get_optimizer(_net, self.beta) for _net in self.anets]

    @cached_property
    def copts(self) -> List[Optimizer]:
        """Critic optimizer"""
        return [get_optimizer(_net, self.alpha) for _net in self.anets]

    def act(self, state: Observation) -> Tuple[Action, List[float]]:
        """Select an action based on state
        ...
        actions, log_probs = zip(
            *[select_action(_net, _state) for _state, _net in zip(state, self.anets)]
        )
        log_probs = [*map(lambda x: x.item(), log_probs)]
    ...
    def update(
        ...
        for _n in range(self.n_players):
            _s0 = torch.from_numpy(state[_n]).float().to('cpu')
            _s1 = torch.from_numpy(next_state[_n]).float().to('cpu')
            _r0 = torch.tensor(next_rewards[_n]).float().to('cpu')
            _mu = torch.tensor(self.mu[_n]).float().to('cpu')
            _lp = torch.tensor(log_probs[_n], requires_grad=True)

            # Compute critic loss
            _v0 = self.cnets[_n](_s0)
            _v1 = self.cnets[_n](_s1)

            _value_loss = loss_func(_r0 - _mu + _v1, _v0)
            _value_loss = torch.clamp(_value_loss, min=-1, max=1)**2
            _value_loss = _value_loss.sum()

            # Compute actor loss
            _advantage = (_r0 - _mu + _v1 - _v0).clamp(min=-1, max=1)
            _actor_loss = _lp * _advantage

            # Backpropagate actor
            self.aopts[_n].zero_grad()
            _actor_loss.backward(retain_graph=True)
            self.aopts[_n].step()

            # Backpropagate critic
            self.copts[_n].zero_grad()
            _value_loss.backward()
            self.copts[_n].step()
            self.mu[_n] += self.zeta * ((_r0 - _mu) + (_v1 - _v0)).clamp(min=-1, max=1).item()
...
if __name__ == "__main__":
    ...
    for episode in trange(config.EPISODES, desc="episodes"):
        ...
        for _ in trange(100, desc="timesteps"):
            ...
            # actor parameters.
            next_actions, next_log_probs = agent.act(next_obs)

            agent.update(obs, actions, next_rewards, next_obs, next_actions, log_probs)

            obs = next_obs
            actions = next_actions
            log_prob = next_log_probs
        ...
        if episode % 1000 == 0 or episode == config.EPISODES - 1:
            ...
            eval_agent = ActorCriticIndependent.load_checkpoint(get_dir(), "current")
            ...
            if eval_rewards / 3200 > best_rewards:
                ...
                test_agent = ActorCriticIndependent.load_checkpoint(
    ...
    for _ in trange(100, desc="timesteps"):
        ...
        next_actions, _ = agent.act(next_obs)
    ...
    agent = ActorCriticIndependent.load_checkpoint(
